// ***********************************************************************************************************************************************************************
// SingleLinkedList.java 			Auteurs: Simard, Mongeau, Desfosses
// 
// Ce fichier contient la classe SingleLinkedList permettant ce créer la liste chainé pour la classe stack.
// NOTE: Il n'était pas nécessaire de le faire de cette façon, on aurait pu utiliser un array comme dans le livre.
// Or, nous trouvions que c'était une bonne idée d'implémenter une LinkedList pour se pratiquer. De toute façon, une autre classe
// utilisera la pile avec array. Pour les classes principales, nous utilisons les deux. (InToPost avec LL et EvaluatePostFix en array.
//********************************************************************************************************************************************************************
package linkedlist;
...
public class SingleLinkedList {
	...
	public void insertionDansLinkedList(char nodeValue, int location) {
		...
		if (!existanceOfLinkedList()) { // Si la linked list n'esxiste pas
		...
		} else if (location >= getSize()) { // Si on entre une position plus grande que la taille, on fait qu'ajouter le
											// noeud a la fin
			node.setNext(null); // Puisque ce n'est qu'une simple linked list et non une simple linked list
								// circulaire, on ne relie pas la queue a la tete
			...
			SingleNode nodeTemporaire = new SingleNode(); // Creation d'un noeud temporaire. Celui-ci nous permettra de
															// relier certains noeud ensembles
			int index = 0;
			...
			/*
			 * Pour l'instant, le noeud temporaire pointe sur le noeud suivant et non le
			 * nouveau noeud. C'est pourquoi on veut creer une variable de type SingleNode
			 * afin de garder une reference sur celui-ci lorsqu'on modifiera le noeud
			 * temporaire pour qu'il pointe sur le nouveau noeud. Imaginons que le noeud
			 * temporaire est a la position 3 et qu'on veut ajouter le nouveau noeud a la
			 * position 4
			 */

			SingleNode nextNode = nodeTemporaire.getNext(); // Le noeud apres le noeud temporaire. Cela servira pour
															// relier le nouveau noeud au suivant(presentement position
															// 4)
			nodeTemporaire.setNext(node); // Le noeud temporaire pointe desormais sur le nouveau noeud et ne pointe plus
											// sur le noeud suivant(position X)
			node.setNext(nextNode); // Le nouveau noeud pointe sur le noeud qui etait a la postion 4. Or, pusiqu'on
									// a relier le noeud temporaire
									// au nouveau noeud, celui-ci est desormais a la position 4 et le 'nextNode' est
									// maintenant a la position 5.
	...
	public void deleteNode(int location) {
		if (!existanceOfLinkedList()) {
			...
			nodeTemporaire.setNext(nodeTemporaire.getNext().getNext()); // Le noeud temporaire pointe deux noeud plus
																		// loin ce qui enleve la reference au noeud
